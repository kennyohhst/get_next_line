...
/*   Updated: 2022/07/28 20:39:59 by kkalika          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line.h"

static char	*killall(char *s1, char *s2, char *s3)
{
	if (s1)
		free(s1);
	if (s2)
...
static int	read_line(char **tline, char **dst, int fd)
{
	if (*tline == NULL)
		return (0);
...
static void	return_line(char **dst, char **tl, char **b)
{
	if (!*tl)
		return ;
...
static char	*two_callocs(char **tmp, char **dest)
{
